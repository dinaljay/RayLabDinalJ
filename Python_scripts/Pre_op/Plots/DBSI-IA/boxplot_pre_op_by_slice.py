...
slice1_data = slice1_data[slice1_data['Visit']== 'Pre_op']
...
slice2_data = slice2_data[slice2_data['Visit']== 'Pre_op']
...
slice3_data = slice3_data[slice3_data['Visit']== 'Pre_op']
...
slice4_data = slice4_data[slice4_data['Visit']== 'Pre_op']
...
for i in range(len(dhi_features)):
    ...
    #slice 1
    sns.boxplot(ax = axes[0,0], x = filter_data1['Patient_Group'],
                     y = filter_data1['Data_Value'], showfliers=False)
    axes[0,0].set_title('Slice 1 - C6')
    axes[0,0].set_ylabel("")
    axes[0,0].set_xlabel("")

    #slice 2
    sns.boxplot(ax = axes[0,1], x = filter_data2['Patient_Group'],
                     y = filter_data2['Data_Value'], showfliers=False)
    axes[0,1].set_title('Slice 2 - C5')
    axes[0,1].set_ylabel("")
    axes[0,1].set_xlabel("")

    #slice 3
    sns.boxplot(ax = axes[1,0], x = filter_data3['Patient_Group'],
                     y = filter_data3['Data_Value'], showfliers=False)
    ...
    #slice 4
    sns.boxplot(ax = axes[1,1], x = filter_data4['Patient_Group'],
                     y = filter_data4['Data_Value'], showfliers=False)
