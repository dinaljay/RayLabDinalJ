...
HP_NUM_UNITS = hp.HParam('num_units', hp.Discrete([12, 25, 50, 100]))
...
with tf.summary.create_file_writer('/media/functionalspinelab/RAID/Data/Dinal/DNN_logs/multi/v1/hparam_tuning').as_default():
    hp.hparams_config(
...
def train_test_model(hparams):
    ...
    model.add(tf.keras.layers.Dense(18, input_dim=18, activation=hparams[HP_ACTIVATION]))
    ...
    tensorboard = TensorBoard(log_dir="/media/functionalspinelab/RAID/Data/Dinal/DNN_logs/multi/v1/{}".format(NAME))
        ...
        for activation in HP_ACTIVATION.domain.values:
            for dropout_rate in (HP_DROPOUT.domain.min_value, HP_DROPOUT.domain.max_value):
                for optimizer in HP_OPTIMIZER.domain.values:
                    ...
                    run('/media/functionalspinelab/RAID/Data/Dinal/DNN_logs/multi/v1/hparam_tuning/' + run_name, hparams)
                    session_num += 1
