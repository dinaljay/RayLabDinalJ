% This script calculates the volume and surface area of the selected ROI_by_slices
...
out_dir_control = '/media/functionalspinelab/RAID/Data/Dinal/MATLAB_Data/DBSI/White_Matter/DHI/Pre_op/ROI_by_slice/Control';
out_dir_mild_csm = '/media/functionalspinelab/RAID/Data/Dinal/MATLAB_Data/DBSI/White_Matter/DHI/Pre_op/ROI_by_slice/Mild_CSM';
out_dir_mod_csm = '/media/functionalspinelab/RAID/Data/Dinal/MATLAB_Data/DBSI/White_Matter/DHI/Pre_op/ROI_by_slice/Moderate_CSM';
...
for k = 1:numel(moderate_cm_subjects)
    ...
    for j = 1:numel(slices)
        ...
        param_file =('dti_fa_map.nii');
        file_name = strcat('JB_CSM_P_S',j,'roi_wm.nii.gz');
        mask_file = fullfile(csm_roi,subjectID,'/scan_1/dMRI_ZOOMit/',slice_num,'/all_volumes/DHI_results_0.3_0.3_3_3/',file_name);        dwi_file = fullfile(csm_path,subjectID,'/scan_1/dMRI_ZOOMit/',slice_num,'/all_volumes/DHI_results_0.3_0.3_3_3',param_file);
...
for i = 1:length(data_mild_csm)
    for j = 1:numel(slices)
        mild_csm_volumes{i,j} = data_mild_csm{i,j}*voxel_volume;
        mild_csm_surface_area{i,j} = data_mild_csm{i,j}*voxel_surface_area;
    end
end
...
for i = 1:length(data_mod_csm)
    for j = 1:numel(slices)
        mod_csm_volumes{i,j} = data_mod_csm{i,j}*voxel_volume;
        mod_csm_surface_area{i,j} = data_mod_csm{i,j}*voxel_surface_area;
    end
end
