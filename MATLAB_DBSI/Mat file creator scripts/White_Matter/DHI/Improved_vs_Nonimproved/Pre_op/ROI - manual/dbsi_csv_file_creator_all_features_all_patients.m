...
in_dir = '/media/functionalspinelab/RAID/Data/Dinal/MATLAB_Data/DBSI/White_Matter/DHI/Improved_vs_Nonimproved';
...
for n=1:numel(dhi_features)
    ...
    temp = fullfile(in_dir,'Pre_op/ROI/Improved/',file_name);
    load(temp);
    improv_pre{n,1} = data_improv;
    
    file_name = strcat('non_improv_',dhi_features(n),'_data.mat');
    temp = fullfile(in_dir,'Pre_op/ROI/Nonimproved/',file_name);
    load(temp);
...
var_Names = ["Patient_ID", "Group", "Group_ID", "b0_map", "dti_adc_map","dti_axial_map", "dti_b_map", "dti_dirx_map", "dti_diry_map", "dti_dirz_map", "dti_fa_map",...
