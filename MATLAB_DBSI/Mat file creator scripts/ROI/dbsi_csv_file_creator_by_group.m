...
out_dir_control = '/media/functionalspinelab/RAID/Data/Dinal/Pycharm_Data_ROI/DBSI_CSV_Data/Control';
out_dir_csm = '/media/functionalspinelab/RAID/Data/Dinal/Pycharm_Data_ROI/DBSI_CSV_Data/CSM';
...
for i = 1:numel(dhi_features)
    ...
    terminal2 = strcat('control_',dhi_features(i),'_data.csv');
    table_out=cell2table(data_control);
    ...
    writetable(table_out,fullfile(out_dir_control,terminal2))
    fprintf('\n')
...
for i = 1:numel(dhi_features)
    ...
    for j = 1:numel(slices)
        ...
        for k = 1:numel(cm_subjects)
            ...
            data_csm{k,j} = mean(dwi_data(expert_rois>0.7));
        end
        
    end
    
    terminal2 = strcat('csm_',dhi_features(i),'_data.csv');
    table_out=cell2table(data_csm);
    table_out.Properties.VariableNames{1} = 'Slice_1';
    table_out.Properties.VariableNames{2} = 'Slice_2';
    table_out.Properties.VariableNames{3} = 'Slice_3';
    table_out.Properties.VariableNames{4} = 'Slice_4';
    
    writetable(table_out,fullfile(out_dir_csm,terminal2));
    fprintf('\n')
...
for i = 1:numel(dhi_features)
    ...
    for j = 1:numel(cm_subjects)
        ...
        for k = 1:numel(slices)
            ...
            data_mild_csm{j,k} = dwi_data(expert_rois>0.7);
        end
        fprintf('\n')
    end
...
for i = 1:numel(dhi_features)
    ...
    for j = 1:numel(moderate_cm_subjects)
        ...
        for k = 1:numel(slices)
            ...
            data_moderate_csm{j,k} = dwi_data(expert_rois>0.7);
        end
        fprintf('\n')
    end
